...
impl BasketManagerConfig {
    ...
    pub fn from_config_data(
        ...
        let (unique_symbols, processed_baskets): (HashSet<Symbol>, Vec<LoadedBasket>) = loaded_baskets
            .into_iter()
            .map(|loaded_basket| {
                ...
                (basket_symbols, LoadedBasket {
                    symbol,
                    basket: Basket::default(), // Use default for metadata storage
                    source_file,
                })
            })
            .fold((HashSet::new(), Vec::new()), |(mut all_symbols, mut baskets), (symbols, basket)| {

...
impl BasketJsonLoader {
    ...
    pub fn load_baskets_from_config(
        ...
        if config_data.indexes_files.is_empty() {
        ...
        let loaded_baskets: Result<Vec<LoadedBasket>, ConfigBuildError> = config_data
            ...
            .iter()
            .map(|mapping| {
                self.load_single_basket(&mapping.symbol, &mapping.file_path)
            })
            .collect();
...
impl BasketManagerConfig {
    ...
    pub fn from_config_data(
        ...
        // Populate the basket manager and collect symbols
        let mut unique_symbols = HashSet::new();

        for loaded_basket in &loaded_baskets {
            // Collect symbols from this basket
            let basket_symbols: Vec<Symbol> = loaded_basket
                .basket
                .basket_assets
                .iter()
            ...
            unique_symbols.extend(basket_symbols);

            // Add basket to the manager
            basket_manager
                .write()
                .set_basket(&loaded_basket.symbol, &Arc::new(loaded_basket.basket.clone()));
        }

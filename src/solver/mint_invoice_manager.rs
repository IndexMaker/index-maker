...
//     #[test]
//     fn migrate_old_format_to_new_format() -> eyre::Result<()> {
...
//         // Read old format
//         let old_data = std::fs::read_to_string(&file_path)?;
//         let old_value: serde_json::Value = serde_json::from_str(&old_data)?;
        
//         // Check if already in new format
//         if old_value.get("metadata").is_some() {
//             println!("✓ File is already in new format. Nothing to do.");
//             return Ok(());
//         }

//         // Parse old format
//         let old_invoices: Vec<(u32, Address, MintInvoice)> = if let Some(invoices) = old_value.get("invoices") {
//             serde_json::from_value(invoices.clone())?
//         } else {
//             println!("No 'invoices' key found. File format unknown.");
//             return Ok(());
//         };

//         println!("Found {} invoices in old format\n", old_invoices.len());

//         // Create manager with new format
//         let persistence = Arc::new(JsonFilePersistence::new(&file_path));
//         let mut manager = MintInvoiceManager::new(persistence);

//         // Add each invoice (creates child files and metadata)
//         for (i, (chain_id, address, invoice)) in old_invoices.into_iter().enumerate() {
//             println!("  [{}/3] Migrating: {}", i + 1, invoice.client_order_id);
...
//         println!("\n=== Migration Complete ===");
//         println!("✓ Created {} child files", manager.metadata.len());
//         println!("✓ Updated root file to new format");
//         println!("✓ Metadata: {} entries", manager.metadata.len());
        
//         // Verify by loading
//         let mut verification_manager = MintInvoiceManager::new(
//             Arc::new(JsonFilePersistence::new(&file_path))
//         );
//         verification_manager.load()?;
        
//         println!("\n=== Verification ===");
//         println!("✓ Loaded {} metadata entries", verification_manager.metadata.len());
        
//         // Try to get one invoice to verify child files work
//         if let Some(first_meta) = verification_manager.metadata.first() {
//             let result = verification_manager.get_invoice(
//                 first_meta.chain_id,
//                 first_meta.address,
...
//             if result.is_some() {
//                 println!("✓ Successfully retrieved invoice from child file");
//             } else {
//                 println!("⚠ Could not retrieve invoice - child file issue");
//             }
//         }

//         println!("\n Migration successful! You can now run regular tests.");

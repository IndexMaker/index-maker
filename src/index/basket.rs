...
impl BasketDefinition {
        ...
        I: IntoIterator<Item = AssetWeight>
    {
        ...
        let total_weight = weights.iter().try_fold(Amount::ZERO, |a, x| a.checked_add(x.weight)).ok_or(eyre!("Numeric overflow"))?;
        let weights = weights.into_iter().map(|w| {
...
impl Display for BasketDefinition {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "BasketDefinition[{}]", self.weights.iter()
...
impl Basket {
    ...
    pub fn new_with_prices(
        ...
        target_price: Amount) -> Result<Basket> {

        // Map definition to locked quantities
        let basket_assets: Vec<BasketAsset> = basket_definition.weights.into_iter().map(|weight| {
            let price: &Amount = individual_prices.get(&weight.asset.name).unwrap_or(&Amount::ZERO);
            let quantity = target_price.checked_div(*price).and_then(|x| x.checked_mul(weight.weight)).unwrap_or_default();
            BasketAsset {
                weight,
                price: *price,
                quantity
        ...
        let unpriced_assets: Vec<&BasketAsset> = basket_assets.iter().filter(|asset| asset.price.is_zero()).collect();
        if !unpriced_assets.is_empty() {
            return Err(eyre!(format!(
                "Unknown price of the assets: {}",
                    unpriced_assets.into_iter().map(|a| a.weight.asset.name.as_ref()).join(", ")
            )));
        }
        
        // Complain about numeric overflow during quantity calculation
        let overflown_assets: Vec<&BasketAsset> = basket_assets.iter().filter(|asset| asset.quantity.is_zero()).collect();
        if !overflown_assets.is_empty() {
            return Err(eyre!(format!(
                "Numeric overflow while computing quantity for assets: {}",
                    overflown_assets.into_iter().map(|a| a.weight.asset.name.as_ref()).join(", ")
            )));
        }
    ...
    pub fn get_current_price(&self, individual_prices: &HashMap<Symbol, Amount>) -> Result<Amount> {
        let prices = self.basket_assets.iter().map(|ba| {
            (&ba.weight.asset.name, individual_prices.get(&ba.weight.asset.name).and_then(|x| x.checked_mul(ba.quantity)))
        }).collect_vec();
        ...
        prices.iter().map(|x| x.1.unwrap())
            .try_fold(Decimal::ZERO, |a, x| x.checked_add(a))
            .ok_or(eyre!("Numeric overflow while computing current price of the basket"))
    }
}

impl Display for Basket {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Basket[{}; {}]", self.target_price,
            self.basket_assets.iter().map(|ba| format!("{:.7}{} @ ${:.7} (w={:.7})", 
                ba.quantity, ba.weight.asset.name, ba.price, ba.weight.weight)).join(", "))
    }
}
...
impl From<Basket> for Vec<AssetWeight> {
    fn from(basket: Basket) -> Self {
        basket.basket_assets.into_iter().map(|ba| ba.weight).collect_vec()
    }
}

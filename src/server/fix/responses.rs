use axum_fix_server::{
    ...
    plugins::{seq_num_plugin::WithSeqNumPlugin, user_plugin::WithUserPlugin, rate_limit_plugin::{MessageType, WithRateLimitPlugin, RateLimitKey}},
};
...
mod tests {
    ...
    fn test_with_rate_limit_plugin_implementation() {
        ...
        assert_eq!(axum_fix_server::plugins::rate_limit_plugin::WithRateLimitPlugin::get_user_id(&ack_response), user_id);
        assert_eq!(ack_response.get_rate_limit_key(), RateLimitKey::User(user_id.0, user_id.1));
        assert_eq!(ack_response.get_message_weight(), 1);
        assert_eq!(ack_response.get_message_type(), MessageType::Administrative);
        
        // Test NAK response
        let nak_response = FixResponse::create_nak(&user_id, &session_id, 124, "Rate limit exceeded".to_string());
        
        assert_eq!(axum_fix_server::plugins::rate_limit_plugin::WithRateLimitPlugin::get_user_id(&nak_response), user_id);
        assert_eq!(nak_response.get_rate_limit_key(), RateLimitKey::User(user_id.0, user_id.1));
    ...
    fn test_response_message_weights() {
        ...
        let test_cases = vec![
            (ResponseBody::ACKBody { ref_seq_num: 1 }, 1, MessageType::Administrative),
            (ResponseBody::NAKBody { ref_seq_num: 1, reason: "Error".to_string() }, 1, MessageType::Administrative),
            (ResponseBody::NewOrderBody { status: "FILLED".to_string(), client_order_id: "O123".to_string() }, 5, MessageType::Order),
            (ResponseBody::NewOrderFailBody { status: "REJECTED".to_string(), client_order_id: "O124".to_string(), reason: "Invalid".to_string() }, 5, MessageType::Order),
            ...
            (ResponseBody::IndexQuoteRequestBody { status: "QUOTED".to_string(), client_quote_id: "Q123".to_string() }, 3, MessageType::Quote),
            (ResponseBody::IndexQuoteRequestFailBody { status: "REJECTED".to_string(), client_quote_id: "Q124".to_string(), reason: "Invalid".to_string() }, 3, MessageType::Quote),
        ];
    ...
    fn test_response_creation_methods() {
        ...
        match nak.body {
            ResponseBody::NAKBody { ref_seq_num, reason } => {
    ...
    fn test_rate_limit_key_consistency() {
        ...
        assert_eq!(response_1.get_rate_limit_key(), response_2.get_rate_limit_key());
        
        // Different users should have different rate limit keys
        assert_ne!(response_1.get_rate_limit_key(), response_3.get_rate_limit_key());
        
        // Verify key structure
        assert_eq!(response_1.get_rate_limit_key(), RateLimitKey::User(1, get_mock_address_1()));
        assert_eq!(response_3.get_rate_limit_key(), RateLimitKey::User(2, get_mock_address_2()));
    }
}

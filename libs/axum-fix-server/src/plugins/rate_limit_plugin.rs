    ...
    
{
    ...
    pub fn new(config: RateLimitConfig) -> Self {
        Self {
            ...
            global_limiter: Arc::new(RwLock::new(
                MultiLimiter::new(config.global_limits.clone())
            )),
    ...
    pub fn check_rate_limits(&self, session_id: &SessionId, user_id: &(u32, Address)) -> Result<(), RateLimitError> {
        ...
        if !self.global_limiter.write().try_consume(1, timestamp) {
            ...
            return Err(RateLimitError::GlobalLimitExceeded {
                retry_after: self.global_limiter.read()
        ...
        if !session_limiter.write().try_consume(1, timestamp) {
            ...
            return Err(RateLimitError::SessionLimitExceeded {
                ...
                retry_after: session_limiter.read()
                    .waiting_period_half_smallest_limit(timestamp),
            });
        }
    ...
    pub fn consume_message_weight(&self, request: &R, _session_id: &SessionId) -> Result<(), RateLimitError> {
        if !self.config.enabled {
            return Ok(());
        }
        ...
        let weight = self.config.message_type_weights
            .get(&message_type)
            .copied()
            .unwrap_or(1);
    ...
    fn get_or_create_session_limiter(&self, session_id: &SessionId) -> Arc<RwLock<MultiLimiter>> {
        ...
        let new_limiter = Arc::new(RwLock::new(
            MultiLimiter::new(self.config.session_limits.clone())
        ));
    ...
    fn get_or_create_user_limiter(&self, user_id: &(u32, Address)) -> Arc<RwLock<MultiLimiter>> {
        ...
        let new_limiter = Arc::new(RwLock::new(
            MultiLimiter::new(self.config.user_limits.clone())
        ));
    ...
    /// Update activity timestamps
    fn update_activity_timestamps(&self, session_id: &SessionId, user_id: &(u32, Address), timestamp: DateTime<Utc>) {
        // Update session metadata
        {
            let mut metadata = self.session_metadata.write();
            metadata.entry(session_id.clone())
                .and_modify(|meta| {
    ...
    pub fn create_session(&self, session_id: &SessionId) -> Result<()> {
        ...
        let mut metadata = self.session_metadata.write();
        metadata.insert(session_id.clone(), SessionMetadata {
            last_activity: timestamp,
    ...
    pub fn start_cleanup_task(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            let mut cleanup_interval = interval(
                TokioDuration::from_secs(config.cleanup_interval_seconds)
            );
...
mod tests {
    ...
    use std::collections::HashMap;
    use chrono::Duration;
    use symm_core::core::{limit::LimiterConfig, test_util::{get_mock_address_1, get_mock_address_2}};
    ...
    fn create_test_config() -> RateLimitConfig {
        ...
        RateLimitConfig {
            session_limits: vec![
                LimiterConfig::new(5, Duration::seconds(60)),
            ],
            user_limits: vec![
                LimiterConfig::new(10, Duration::seconds(60)),
            ],
            global_limits: vec![
                LimiterConfig::new(20, Duration::seconds(60)),
            ],
            message_type_weights: message_weights,
    ...
    async fn test_rate_limit_enforcement() {
        ...
        if let Err(RateLimitError::SessionLimitExceeded { session_id: err_session, .. }) = result {
    ...
    async fn test_message_weight_consumption() {
        ...
        timestamp += chrono::TimeDelta::milliseconds(100); // Same window
        let result = plugin.consume_message_weight_at_time(&request, &session_id, timestamp);
        assert!(result.is_err(), "Third request should fail due to weight limit");
        
        // ✅ Test 4: Verify error type
        match result {
            Err(RateLimitError::UserLimitExceeded { user_id: err_user, weight, .. }) => {
